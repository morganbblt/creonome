...
export type OpportunityRecord = {
  ...
  position: number;
  /**
   * The single source of truth for which of the three creative strategies
   * this opportunity represents. Written once at generation time (see
   * neon-opportunities.repository.ts#createBatch) and read back verbatim
   * here -- the API layer must never re-derive this from `position`.
   */
  ...
  availableAt: Date;
  /**
   * True when the batch this opportunity belongs to was generated by the
   * local fallback path instead of the structured AI generator. Carried
   * per-row alongside the other batch-level field `availableAt` so callers
   * that already iterate `OpportunityRecord[]` don't need a second query.
   */
  fallback: boolean;
...
export type CreateOpportunityBatchInput = {
  ...
  /**
   * True when `opportunities` came from the local fallback set (see
   * opportunity-generation.service.ts#localOpportunities) because the
   * structured AI generator failed, rather than a real generation. Written
   * onto `opportunity_batches.fallback` so `OpportunityBatchSchema.fallback`
   * can disclose it honestly instead of silently presenting canned copy as
   * a live generation.
   */
  fallback: boolean;
};
